...
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Figure 12.5 Sampling distribution of the F-ratio"
...
ggplot(data, aes(x = x, y = y)) +
  ...
  labs(
    title = "Sampling Distribution of the F-ratio",
    x = "F value",
    y = ""
  ) +
  ...
  annotate(
    ...
    x = f_crit + 0.2,
    y = max(y) * 0.8,
    label = paste0("Critical value = ", round(f_crit, 2))
  ) +
  
  theme_minimal(base_size = 14) +
  
  # Remove extra margins between axes and plot
...
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Figure 12.6 Sampling distribution of the t-statistic"
...
ggplot(data, aes(x = x, y = y)) +
  ...
  labs(
    x = "t value",
    y = "",
    title = "Sampling Distribution of the t-statistic"
  ) +
  ...
  annotate(
    ...
    x = 2.5,
    y = max(y) * 0.8,
    label = paste0("Critical values = ±", round(t_crit, 2))
  ) +
  
  theme_minimal(base_size = 14) +
  
  # Remove margins between axes and plot
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
...
#| fig-width: 6
#| fig-height: 6
#| fig-cap: "Figure 12.7 Three-variable overlap diagram"

library(ggplot2)

# Create circle data manually
circle <- function(center = c(0,0), r = 1, n = 200){
...
# Define circles
c1 <- circle(c(-1, 0.5), 1.8)   # Y (left)
c2 <- circle(c(1, 0.5), 1.8)    # X1 (right)
c3 <- circle(c(0, -1), 1.8)     # X2 (bottom)

# Plot
ggplot() +
  geom_polygon(data = c1, aes(x, y), fill = "blue", alpha = 0.4) +
  geom_polygon(data = c2, aes(x, y), fill = "red", alpha = 0.4) +
  geom_polygon(data = c3, aes(x, y), fill = "yellow", alpha = 0.4) +
  
  # Labels inside regions
  annotate("text", x = -1.8, y = 1.2, label = "A", size = 6) +
  annotate("text", x = 0, y = 1.3, label = "B", size = 6) +
  annotate("text", x = 0, y = 0, label = "C", size = 6) +
  annotate("text", x = -0.8, y = -0.8, label = "D", size = 6) +
  
  # Circle labels
  annotate("text", x = -3, y = 0.5, label = "Y", size = 5) +
  annotate("text", x = 3, y = 0.5, label = "X₁", size = 5) +
  annotate("text", x = 0, y = -3, label = "X₂", size = 5) +
...
For our example, the coefficient of determination ($R^2$) is calculated as follows: 
...
#### Variance Inflation Factors (VIF)  {.unnumbered}
...
### Reporting Results of a Multiple Regression Analysis  
