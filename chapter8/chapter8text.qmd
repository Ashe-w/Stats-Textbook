...
#| fig-width: 7
#| fig-height: 4.2
#| fig-cap: "Histogram of ElaSS"
# Approximated values to match the histogram counts
ElaSS <- c(
  315, 335,                         # 300–350
  355, 365, 372, 380, 390, 395, 398,   # 350–400
  402, 408, 412, 418, 420, 425, 428, 432, 435,
  438, 442, 445, 448, 450, 455, 458, 462,  # 400–450
  468, 472, 478, 485, 490, 495, 498,      # 450–500
  515, 535                               # 500–550
)
...
# Convert t values to sample mean scale
se <- 7.1   # approximate SE used to position critical/sample points visually
x_crit <- mu + t_crit * se
x_sample <- mu + sample_stat * se

# Generate distribution
x <- seq(410, 470, length.out = 500)
y <- dt((x - mu)/se, df=df)

data <- data.frame(x, y)

# Rejection region
reject <- subset(data, x <= x_crit)

ggplot(data, aes(x, y)) +
  geom_line(color="blue", linewidth=1) +
  
  geom_area(data=reject, aes(x,y),
            fill="blue", alpha=0.2) +
  
  geom_vline(xintercept = mu,
             color="red", linetype="dashed") +
  
  geom_vline(xintercept = x_crit,
             color="black", linetype="dashed") +
  
  annotate("text", x = x_crit - 2, y = max(y)*0.15,
           label = "-1.69",
           fill="white", label.size=0.3) +
  
  annotate("text", x = x_sample - 2, y = max(y)*0.05,
           label = "-1.938",
           fill="white", label.size=0.3) +
  
  scale_x_continuous(limits=c(410,470), expand=c(0,0)) +
...
![Fig 8.6: Applet demo](chapter8/fig8-6.png){fig-align="center" width=60%}
...
A chi-square goodness-of-fit test was conducted to determine whether the state-level STEM enrichment program provides equitable access based on gender, with expected proportions for male and female participants. All assumptions for the analysis were met. The results indicated no statistically significant difference in the proportions of males and females selected for the program ($\chi^2(1) = 3.72, \, p=0.054$). These findings suggest that the enrichment program provides equitable access by gender.   
