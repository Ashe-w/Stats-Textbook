...
# Understanding the Center: Measures of Central Tendency {.unnumbered}
...
The **mean** is the most commonly reported measure of central tendency in educational statistics. It is the arithmetic average of a dataset.  More specifically, a sample mean, often denoted as $\bar{X}$ can be defined as $\bar{X} = \frac{\sum_{i=1}^{n} x_i}{n}$  where *n* represents the number of cases and $x_i$ represents the *i*th value of variable *x*. In other words, $\sum_{i=1}^{n} x_i$ means the sum of first to *n*th values of x. The sample mean is said to estimate $\mu$ which is the population mean. For our score data, the mean is: 
$$
...
1. Adding a new score or removing an existing score will change the mean, unless the value is equal to the mean. For example, adding a score of 1 to our dataset would change the mean to 12.  

2. Changing an existing score will change the mean. For example, changing the first 10 in our dataset to 6 would change the mean to 12.9. 

3. Adding, subtracting, multiplying, or dividing each score by a constant will change the mean by that constant. For example, if we added 100 to each score in our sample, the new mean would be 113.4.  

4. The sum of the differences of scores from their mean is equal to 0. The table below illustrates this for our example. Note that the exact mean of 13.375 is used to avoid rounding error.  
    ...
    **Sum of the Difference of Scores from the Mean**
    ...
    | $16$ | $16 - 13.375 = 2.625$ |
    | $18$ | $18 - 13.375 = 4.625$ |
    | **$\sum x_i = 107$** | **$\sum (x_i - \bar{X}) = 0$** |


5. The sum of squared distances of scores from the mean is minimal, meaning no other value would lead to a lower sum of squared distances. Using squared distances will always produce a positive number. The larger the final product, the further scores deviate from the mean. The smaller the final product, the less scores deviate from the mean. 
    ...
    **Sum of Squared Difference of Scores from the Mean**
    ...
    | $16$ | $(16 - 13.4)^2 = 6.76$ |
    | $18$ | $(18 - 13.4)^2 = 21.16$ |
    | **$\sum x_i = 107$** | **$\sum (x_i - \bar{X})^2 = 59.88$** |
...
## Advantages and Disadvantages of the Mode, Median and Mean 
|               | **Mode** | **Median** | **Mean** |
|---------------|----------|------------|----------|
| **Advantages** | Represents the most frequently occurring number<br><br>Applicable to nominal data<br><br>Not affected by outliers | Not affected by outliers<br><br>Does not assume the scale is interval | Can be manipulated algebraically (i.e., inferential analyses are possible)<br><br>Uses all data to compute<br><br>Allows inference about population characteristics<br><br>More stable with large data |
| **Disadvantages** | Does not represent the entire collection of data | Difficult to apply inferential analysis to the median | Influenced by outliers<br><br>Requires interval properties of the data |
...
#| fig-width: 8
#| fig-height: 5.4
#| fig-cap: "Figure 2.1 Negatively skewed distribution with mean, median, and mode"

library(ggplot2)

# Generate negatively skewed data (mirror of gamma)
set.seed(1)
raw <- rgamma(1000, shape = 1.8, scale = 0.75)
data <- data.frame(value = 8 - raw)  # flip to create left skew

# Match displayed summary values
mean_val <- 6.63
median_val <- 6.98
mode_val <- 7.27
...
ggplot(data, aes(x = value)) +
  ...
  annotate(
    "segment", x = 0.2, xend = 0.6, y = 0.68, yend = 0.68,
    linetype = "dashed", color = "red", linewidth = 0.8
  ) +
  annotate(
    "segment", x = 0.2, xend = 0.6, y = 0.64, yend = 0.64,
    linetype = "dashed", color = "darkgreen", linewidth = 0.8
  ) +
  annotate(
    "segment", x = 0.2, xend = 0.6, y = 0.60, yend = 0.60,
    linetype = "dashed", color = "purple", linewidth = 0.8
  ) +
  annotate(
    "text", x = 0.65, y = 0.68, label = "Mean: 6.63",
    hjust = 0, vjust = 0.5, size = 3.2
  ) +
  annotate(
    "text", x = 0.65, y = 0.64, label = "Median: 6.98",
    hjust = 0, vjust = 0.5, size = 3.2
  ) +
  annotate(
    "text", x = 0.65, y = 0.60, label = "Mode: 7.27",
    hjust = 0, vjust = 0.5, size = 3.2
  ) +
  scale_x_continuous(
    limits = c(0, 8),
...
#| fig-width: 8
#| fig-height: 5.4
#| fig-cap: "Figure 2.2 Normal distribution with mean median mode aligne."
...
ggplot(data, aes(x = value)) +
  ...
  geom_vline(
    aes(xintercept = 0, linetype = "Mean/Median/Mode: 0.00"),
    color = "red",
    linewidth = 1
  ) +
  scale_color_manual(values = c("Normal Distribution" = "black")) +
  scale_linetype_manual(values = c("Mean/Median/Mode: 0.00" = "dashed")) +
  scale_x_continuous(
    limits = c(-4.5, 4.5),
    breaks = seq(-4, 4, 2)
  ) +
...
#| fig-width: 8
#| fig-height: 5.4
#| fig-cap: "Figure 2.3 Positively skewed distribution with mean, median, and mode."

library(ggplot2)

# Approximate positively skewed data
set.seed(1)
data <- data.frame(
  value = rgamma(1000, shape = 1.8, scale = 0.75)
)
...
ggplot(data, aes(x = value)) +
  ...
  annotate(
    "segment", x = 6.55, xend = 6.95, y = 0.675, yend = 0.675,
    linetype = "dashed", color = "red", linewidth = 0.8
  ) +
  annotate(
    "segment", x = 6.55, xend = 6.95, y = 0.635, yend = 0.635,
    linetype = "dashed", color = "darkgreen", linewidth = 0.8
  ) +
  annotate(
    "segment", x = 6.55, xend = 6.95, y = 0.595, yend = 0.595,
    linetype = "dashed", color = "purple", linewidth = 0.8
  ) +
  annotate(
    "text", x = 7.02, y = 0.675, label = "Mean: 1.37",
    hjust = 0, vjust = 0.5, size = 3.2
  ) +
  annotate(
    "text", x = 7.02, y = 0.635, label = "Median: 1.02",
    hjust = 0, vjust = 0.5, size = 3.2
  ) +
  annotate(
    "text", x = 7.02, y = 0.595, label = "Mode: 0.73",
    hjust = 0, vjust = 0.5, size = 3.2
  ) +
  scale_x_continuous(
    limits = c(0, 8),
...
#| message: false
#| warning: false
#| fig-cap: "Figure 2.4: Example of a Bimodal Distribution"
#| fig-align: center
#| fig-width: 7.5
#| fig-height: 4
...
# Create a smooth bimodal curve using two normal densities
x <- seq(1, 10, length.out = 500)
y <- 5 * dnorm(x, mean = 3, sd = 1) +
     5 * dnorm(x, mean = 7, sd = 1)
...
#| message: false
#| warning: false
#| fig-cap: "Figure 2.5: Example of a Multimodal Distribution"
#| fig-align: center
#| fig-width: 7.5
#| fig-height: 4
...
# Create a smooth multimodal curve using three normal densities
x <- seq(1, 10, length.out = 600)

y <- 8 * dnorm(x, mean = 3, sd = 0.6) +
     8 * dnorm(x, mean = 5, sd = 0.6) +
     8 * dnorm(x, mean = 8, sd = 0.6)
...
#| message: false
#| warning: false
#| fig-cap: "Figure 2.6: Example of Nonmodal or Uniform Distribution"
#| fig-align: center
#| fig-width: 7.5
#| fig-height: 4
...
# Create uniform test score data (each score appears equally often)
scores <- rep(1:4, each = 3)
df <- data.frame(score = scores)

ggplot(df, aes(x = factor(score))) +
  geom_bar(
    fill = "#5FA8A8",
    color = "black",
    width = 0.8
